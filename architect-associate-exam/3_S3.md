...
# S3 Object Lock
![s3ObjectLocks.png](img/s3ObjectLocks.png)
- Object lock is only for new buckets. Auto versioning is enabled.
...
- Per bucket set defaults, but can override per object

Two protections:
- Object can have 1 or 2 or none
...
- Compliance. "The only way to delete an object under the compliance mode before its retention date expires is to delete the   
  associated AWS account." Deep freeze. Everything is immutable.
...
# S3 Access Points
S3 Access point = simplify access to S3 buckets
S3 access points = mini-buckets
S3 access points = diff rules for diff spring boot endpoints
...
- Solution: each team gets its own access point (URL), with own resource policy, also limits on prefix
...
- You can also set a VPC origin as an allowed entry for traffic. Need a VPC endpoint.
- Main bucket resource policy must be kind of in sync with these delegated policies